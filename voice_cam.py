...
if __name__ == '__main__':
    ...
    for path in glob.glob('./TEST_DATA/*'):
            ...
            for times in range(5):
                # Get mfccs
                mfccs_tensor = torch.tensor(mfccs, dtype=torch.float).unsqueeze(0).unsqueeze(0) # 1 * 1 * 40 * 100
                ...
                for i in range(len(map)):
                    for j in range(len(map[0])):
                        ...
                        sum += mfccs[i][j]
                        dic[int(mfccs[i][j])] = dic[int(mfccs[i][j])] + 1
                        if dic[int(mfccs[i][j])] > cnt:
                            cnt = dic[int(mfccs[i][j])]
                            most = mfccs[i][j]
                # Sorting 
